    ...
    (decline ()
      (let ((off (offset layout))
            (dx (case (limit layout)
                  (:x 0.0f0)
                  (:y (if (= 0.0f0 (dx event)) (dy event) (dx event)))
                  (T (dx event))))
            (dy (case (limit layout)
                  (:x (if (= 0.0f0 (dy event)) (dx event) (- (dy event))))
                  (:y 0.0f0)
                  (T (- (dy event))))))
        (setf (offset layout) (px-point (+ (* (pxw layout) 0.1f0 dx) (pxx off))
                                        (+ (* (pxh layout) 0.1f0 dy) (pxy off))))))))
...
(defmethod ensure-visible ((element layout-element) (layout clip-view))
  (let* ((view (bounds layout))
    ...
    (cond ((< (pxy element) vyb)
           (setf (offset layout) (px-point 0.0f0 (- (pxy element)))))
          ((< vyt (+ (pxy element) (pxh element)))
           (setf (offset layout) (px-point 0.0f0 (- (pxh view) (pxy element) (pxh element)))))))
  (call-next-method))
