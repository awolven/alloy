...
(defclass font (simple:font)
  ((renderer :initarg :renderer :accessor renderer)
   ...
   (px-range :initform 4.0f0 :accessor px-range)
   (data :accessor data)
...
(defun map-glyphs (font-sequence function string extent scale wrap &key start end (halign :start))
  ...
  (let ((x 0.0f0) (y 0.0f0)
        ...
        (last-width 0.0f0)
        (base-scale scale)
        f (w 0.0f0) (h 0.0f0) line (space 0.0f0) chars kernings
        (%k (cons NIL NIL))
        (i (or start 0)) (si 0)
        (end (or end (length string))))
               ...
               (unless (eq f font)
                 ...
                 (let ((font (data font)))
                   (setf scale (* (/ base-scale (3b-bmfont:base font))
                                  (/ #++(3b-bmfont:size font) 32.0f0 35.0f0)))
               ...
               (loop for i from start below end
                     ...
                     do (find-font i)
                        (case c
                          ...
                          (#\space
                           (incf x space)
                           (funcall function i x y 0.0f0 0.0f0 0.0f0 0.0f0 0.0f0 0.0f0 0.0f0 0.0f0))
                          (t
...
(defun compute-text (font text extent scale wrap markup halign)
  ...
  (let ((array (make-array (* 6 10 (the (signed-byte 32) (length text))) :element-type 'single-float))
        (minx 0.0f0) (miny 0.0f0) (maxx 0.0f0) (maxy 0.0f0) (i 0)
    ...
    (labels ((prop (prop styles)
             ...
             (vertex (x y u v c)
               ...
               (setf (aref array (+ i 8)) 0.0f0)
               (setf (aref array (+ i 9)) 0.0f0)
               (incf i 10))
             (thunk (c x y x- y- x+ y+ u- v- u+ v+)
               ...
               (let ((tx (if (prop :italic styles) (* (/ (- y+ y-) base) 15.0f0) 0.0f0))
                     (off (if (prop :bold styles) (* scale 0.2f0) 0.0f0))
                     (color NIL))
...
(defmethod org.shirakumo.alloy.animation:update :after ((text text) dt)
  (let* ((markup (markup text))
    ...
    (when markup
      (incf clock dt)
      (setf (clock text) (mod clock 36.0f0))
        ...
        (loop with i = 0
              ...
              do (when (<= (or (caar markup) (length data)) c)
                 ...
                 (when (prop :rainbow (rest style))
                   (let ((color (colored:convert (colored:hsv (mod (+ (* 10 c) (* 200 clock)) 360.0f0) 1 1) 'colored:rgb)))
                     (loop for j from 0 below 6
                           do (setf (aref data (+ 4 (* 10 (+ j (* 6 i))))) (colored:r color))
                              ...
                              (setf (aref data (+ 7 (* 10 (+ j (* 6 i))))) 1.0f0))))
                 (when (prop :wave (rest style))
                   (let* ((s (scale text))
                          (off (* s 10 (sin (+ (* 0.5f0 c) (* 10 clock))))))
                     (loop for j from 0 below 6
                           do (setf (aref data (+ 9 (* 10 (+ j (* 6 i))))) off))))
                 (when (prop :shake (rest style))
                   (let* ((s (* 0.02f0 (scale text)))
      ...
      (let ((sequence (font-sequence shape)))
        (loop for i from 0 below (1- (length sequence))
              ...
              do (let ((start (* 6 start))
                       (end (min (* 6 end) count)))
                   (setf (opengl:uniform shader "pxRange") (* (sqrt 2.0f0) (px-range font)))
...
(defmethod alloy:suggest-size (size (text text))
  (let ((scale (alloy:to-px (simple:size text)))
    ...
    (multiple-value-bind (breaks array font-sequence x- y- x+ y+)
        (compute-text (simple:font text) (alloy:text text) size (coerce scale 'single-float) (simple:wrap text) markup (simple:halign text))
      (let* ((w (- x+ x-))
             ...
             (p (simple:resolve-alignment (simple:bounds text) :start (simple:valign text)
                                          (alloy:px-size w h))))
        (values (alloy:px-extent (alloy:pxx p) (+ (- h line) (alloy:pxy p)) w (if (<= (length breaks) 1) h (+ h (* scale 0.5f0))))
